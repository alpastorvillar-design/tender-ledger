...
class VisibilityTests(LoaderTestCase):
    def test_reader_keeps_previous_capture_until_replacement_publishes(self):
        ...
        begin = repo.begin_capture(self.conn, "daily/z", sha, size)
        ...
        rows = [project_member(m) for m in stream_notices(pkg_v2)]
        repo.load_batch(self.conn, begin.capture.capture_id, 0, rows)
    ...
    def test_failed_publish_transaction_changes_nothing(self):
        ...
        load_package(self.conn, pkg_v1, "daily/q")
        ...
        with self.assertRaises(RuntimeError):
            load_package(conn2, pkg_v2, "daily/q", before_publish=boom)
...
class FailureTests(LoaderTestCase):
    ...
    def test_failure_after_a_committed_batch_keeps_it_internal_and_resumable(self):
        ...
        begin = repo.begin_capture(self.conn, "daily/big", sha, size)
        from tender_ledger.packages import stream_notices
        from tender_ledger.projection import project_member
        rows = [project_member(m) for m in stream_notices(pkg)]
        repo.load_batch(self.conn, begin.capture.capture_id, 0, rows[:2])
        # crash: connection lost before further batches / publish
        self.conn.close()
...
class ConcurrencyTests(LoaderTestCase):
    def test_second_capture_of_the_same_package_is_refused_while_one_is_open(self):
        ...
        repo.begin_capture(holder, "daily/lock", sha, size)
    ...
    def test_different_packages_capture_concurrently(self):
        ...
        holder = self.new_conn()
        repo.begin_capture(holder, "daily/a", sha_a, size_a)
...
class RecoveryEquivalenceTests(LoaderTestCase):
    def test_recovered_state_matches_a_clean_run_over_the_same_artifact(self):
        ...
        sha, size = _digest(pkg)
        c1 = self.new_conn()
        begin = repo.begin_capture(c1, "daily/interrupted", sha, size)
        from tender_ledger.packages import stream_notices
        from tender_ledger.projection import project_member
        rows = [project_member(m) for m in stream_notices(pkg)]
        repo.load_batch(c1, begin.capture.capture_id, 0, rows[:3])
        c1.close()
...
def _digest(path):
    from tender_ledger.loader import digest_archive
